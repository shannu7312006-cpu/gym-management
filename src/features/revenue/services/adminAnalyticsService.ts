...
export class AdminAnalyticsService {
  static async getStats(db = defaultDb) {
  ...
  static async getClassUtilisation(limit = 10, db = defaultDb) {
    const rows = await db
      .select({
        id: classes.id,
        name: classes.name,
  ...
  static async getRevenueByMonth(db = defaultDb) {
    const rows = await db
      .select({
  ...
  static async getRevenueByMethod(db = defaultDb) {
    const rows = await db
      .select({
  ...
  static async getExpiringMemberships(db = defaultDb) {
    const today = new Date().toISOString().slice(0, 10);
  ...
  static async getRefundCount(db = defaultDb) {
  ...
  static async getCheckinsPerDay(db = defaultDb) {
  ...
  static async getTopTrainers(db = defaultDb) {
    const start = new Date();
    start.setDate(start.getDate() - 14);
    const startStr = start.toISOString().slice(0, 10);
  ...
  static async getNoShowList(db = defaultDb) {
    const start = new Date();
    start.setDate(start.getDate() - 14);
    const startStr = start.toISOString().slice(0, 10);
    ...
    const trainersMap = new Map<number | null, string>();
    ...
    return rows.map((r) => ({
      ...
      trainerId: r.trainerId,
      trainerName: r.trainerId ? trainersMap.get(r.trainerId) : undefined,
    }));
  }
}
